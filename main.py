...
class GameScreen(Screen):
    def __init__(self, **kwargs):
        ...
        self.layout.add_widget(self.lives_lbl)
        self.layout.add_widget(self.question_lbl)
    ...
    def start_round(self, *args):
        ...
        if self.lives <= 0:
            ...
            # شاشة النهاية كتعرض النتيجة الحالية وأحسن نتيجة
            self.question_lbl.text = f"GAME OVER!\nScore: {self.score}\nBest: {high_score}"
            self.question_lbl.font_size = '28sp'
            Clock.schedule_once(lambda dt: self.go_to_dict(), 3.5)
            return

        self.round_active = True
        self.question_lbl.font_size = '38sp'
        ...
        for i, btn in enumerate(self.options):
            ...
            btn.color = (1, 0.8, 0, 1) # إعادة اللون الأصفر الافتراضي للكلمات
    ...
    def check_collision(self, anim, widget):
        ...
        # تحديد الكلمة الصحيحة والكلمة اللي صطدم بيها اللاعب
        ...
        # تطبيق تلوين الإجابات بالخضر والحمر
        if chosen_lbl == correct_lbl:
            self.score += 10
            if correct_lbl: correct_lbl.color = (0, 1, 0, 1) # أخضر للإجابة الصحيحة
        else:
            self.lives -= 1
            if chosen_lbl: chosen_lbl.color = (1, 0, 0, 1)   # أحمر للإجابة الخاطئة
            if correct_lbl: correct_lbl.color = (0, 1, 0, 1) # أخضر لتوضيح الإجابة الصحيحة
            
        self.update_ui()
        if self.game_active:
            # مهلة ثانية كاملة (1.0) باش يمديك تشوف الألوان قبل ما تبدا الجولة التالية
...
if __name__ == '__main__':
    WordMasterApp().run()# YOUR MAIN.PY FILE IS HERE, REPLACE IT HERE.
